...
Work inside the shared environment. Activate it with `conda activate helper` so your prompt shows `(helper)`. If you have not created it yet, see `docs/onboarding/04_environments.md`. Confirm the repository lives in a normal folder on your machine and not inside OneDrive, iCloud, or Dropbox, which corrupt Git repositories. See `docs/onboarding/03_getting_started_with_git.md`.

## The workflow

Every change, even a one-line fix, follows the same loop: pull the latest code, make a branch, do the work, commit, push, and open a pull request. The full loop with both the VS Code Source Control panel and the equivalent terminal commands is in `docs/onboarding/05_daily_workflow.md`. We use branches for every change because they keep the `main` branch working and give you a safe place to experiment.
