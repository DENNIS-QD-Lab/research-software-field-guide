...
The scheme tells a user, at a glance, whether upgrading is safe. There is also the **0.x convention**: while a project is below `1.0.0`, it is signaling "still unstable, anything may change." A project moves past that once it cuts a `v1.0` release; later work then continues on the `2.x` line, `3.x` line, and so on.
