...
- **Does it have a docstring?** Every script needs purpose, inputs, and an example call at the top. See `06_adding_a_script.md`.
...
To merge, return to the Conversation tab and click "Merge pull request," then "Confirm merge." Delete the branch when GitHub offers, since its work now lives in `main`. The author then pulls `main` to bring the merged change back to their machine, and the loop in `05_daily_workflow.md` starts again.
