...
5. **Update the docs index if needed.** If your helper adds a new category worth indexing, add a line for it in `../README.md`.

6. **Commit, push, and open a pull request.** Write a description that explains what the helper does and includes an example invocation. The format is in `05_daily_workflow.md`.

7. **Review a labmate before merging your own.** Find one open pull request from someone else and leave one substantive comment: a question, a suggestion, or "looks good, and here is why." This is the other half of the workflow, and `08_code_review.md` covers how to do it well.
