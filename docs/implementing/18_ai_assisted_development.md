...
Using AI appropriately in research comes down to the division of labor: **AI writes code; you do the science.** Designing the experiment, judging whether a result is real, and drawing the conclusion are the scientist's responsibility; the assistant is a tool that makes building and running the experiment more efficient. Maintaining these clear roles keeps the work trustworthy, and it matters *more*, not less, the more of the implementation you hand over.

> A note about AI eagerness... The AI assistants are trained to be helpful. They want to complete the job, not fully understanding your good science goals. They are also talkative. They will add words (_so many words_) to your experiment READMEs, will try to make figure and manuscript outlines for you and fill in all kinds of context on your science; they will quickly fill your repo lab notebook with so. much. content. At first this seems so helpful-- you get to keep pushing through the experiments while the AI assistant writes all the commentary! But experience shows that what the AI assistant writes is typically close to, but not quite, what the human scientist would write. Slowly, the description of the hypothesis or plan drifts from what you intend, the experiments start to drift from what you intend, and after awhile the whole project is drifting. (This plus many scientists and educators feel strongly that struggling through these early versions of articulating your hypotheses, goals, and observations builds to struggling through generating your first drafts of your manuscript or dissertation, which is how we learn how to do science as well as how to communicate our scientific ideas. Outsourcing the documentation is outsourcing the thinking-- convenient in the moment and terribly detrimental in aggregate over the long term.).  
>
> In this context, after some iteration, the repo instructions tell the AI assistants to back off and leave more of the commentary to the human scientists. It is your job as a human scientist to hold this line and keep reminding them to stay in their lane as the actions drift from this instruction. 

## The standards file does the heavy lifting

Your project's coding-standards file (here, [CLAUDE.md](../../CLAUDE.md)) states the standards, and the assistant reads it automatically at the start of every session. That is why generated code already follows PEP 8, uses NumPy-style docstrings and type hints, and respects the data-handling rules, without anyone re-explaining them. Keeping standards in a file the assistant reads means the whole team gets consistent output and you are not re-typing the rules in every new chat/coding session. Most assistants support analogous project-instructions file of some kind.

## Three homes for instructions: state, procedure, standards

A standards file is one of *three* kinds of instruction suggested here for a research repo:
...
- **Durable procedure — for the assistant.** How *this* repo runs and records experiments (the run-provenance and folder conventions of [16_running_a_dry_lab_experiment.md](16_running_a_dry_lab_experiment.md)), written down so the assistant follows the same discipline every session instead of improvising. This is a short `experiments_playbook.md` that the assistant reads alongside the standards file. As you have edits to the repo structure or experiment templates, etc., to better suit the needs of your particular project, these details can be added to the `experiments_playbook.md` for the AI assistant to follow.
...
- **Seed and record.** AI-generated code is no exception to the seeding and provenance rules ([16_running_a_dry_lab_experiment.md](16_running_a_dry_lab_experiment.md)). Knowing the seed is still required to precisely replicate a previous result.
