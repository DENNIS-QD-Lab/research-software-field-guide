...
For example, a pipeline organized this way puts its importable code under `src/<package>/` with a `pyproject.toml` declaring its `name` and version. The SWIR_HDR project is a worked instance: `14_experiments_and_shipping.md` moved it to exactly this shape, with `src/swir_hdr/` and a `pyproject.toml` declaring `name = "swir_hdr"` and its version.
