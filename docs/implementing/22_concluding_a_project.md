...
1. **Freeze** the exact state behind the result. Tag it (`paper-v1`;
   `21_versioning_and_releases.md`) so that commit is pinned no matter how the code moves afterward.
...
moving forward on `main` (`15_experiments_and_shipping.md`). Reproducibility comes from the archived
...
- **Zenodo** mints a **DOI** (a permanent identifier) for a snapshot of your repository. Connect the
  ...
  `21_versioning_and_releases.md`, is archived and assigned its own DOI. The paper cites the DOI of
...
Concluding a project is also where the two jobs of the repo (`15_experiments_and_shipping.md`) part
