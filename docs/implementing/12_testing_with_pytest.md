...
- **When an experiment establishes that an approach is correct, turn it into a test.** "This input should produce this result" becomes a `test_` function guarding the shipped code forever. This is how a validation experiment graduates into a permanent safeguard, and it depends on the experiment being reproducible in the first place (the seeding and pinning discussed in `10_from_scripts_to_pipelines.md` and `15_experiments_and_shipping.md`). It is the doc-14 exercise done on real, meaningful code.
