...
- **The assistant's interpretation, presented as if it were yours.** Asked to describe a result, an assistant will often also assert what it *means* or recommend what to do next — fluently, but not necessarily correctly. Keep that judgment call out of the assistant's own prose; it belongs in your own signed blockquote instead ([16_running_a_dry_lab_experiment.md](16_running_a_dry_lab_experiment.md)), even if that means leaving a `_pending_` placeholder rather than an answer filled in for you.
